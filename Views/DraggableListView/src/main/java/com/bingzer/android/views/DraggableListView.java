...
public class DraggableListView extends ListView {
    ...
    public void setAdapter(ListAdapter adapter) {
        if(!(adapter instanceof DraggableListAdapter) && !isInEditMode())
            throw new IllegalArgumentException("Expecting DraggableListAdapter");
    ...
    private void handleCellSwitch() {
        ...
        if (isBelow || isAbove) {
            ...
            final List objectList = ((DraggableListAdapter) getAdapter()).getObjectList();
            swapElements(objectList, originalItem, getPositionForView(switchView));
    ...
    private void swapElements(List arrayList, int indexOne, int indexTwo) {
        Object one = arrayList.get(indexOne);
        Object two = arrayList.get(indexTwo);

        arrayList.set(indexOne, two);
        arrayList.set(indexTwo, one);

        if(getAdapter() instanceof DraggableListAdapter){
            ((DraggableListAdapter) getAdapter()).onSwapped(two, indexOne);
            ((DraggableListAdapter) getAdapter()).onSwapped(one, indexTwo);
        }
    }
